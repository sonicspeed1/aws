...
import { ImpuestoService } from '../impuesto.service';  // Cambiado desde GastoService
  ...
  calcularImpuesto() {
    ...
    if (this.salud <= limiteSalud && this.vivienda <= limiteVivienda && this.educacion <= limiteEducacion && this.vestimenta <= limiteVestimenta && this.alimentacion <= limiteAlimentacion) {
      ...
      if (this.gastosestado(this.totl)) {
        ...
        this.impuestoService.createimpuestos(nuevoImpuesto).subscribe(
          response => {
            this.mensaje += ' Impuesto guardado con éxito!';
          },
          error => {
            this.mensaje += ' Error al guardar el impuesto.';
          }
        );
